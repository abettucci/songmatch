...
-- Concert companion MVP. This mirrors migrations/20260922_add_concert_companions.sql
-- for freshly initialized Docker volumes.
CREATE TABLE IF NOT EXISTS user_profiles (
    user_id UUID PRIMARY KEY REFERENCES users(id) ON DELETE CASCADE, display_name VARCHAR(50) NOT NULL,
    bio VARCHAR(280) NOT NULL DEFAULT '', city VARCHAR(80) NOT NULL, public_interests TEXT[] NOT NULL DEFAULT '{}',
    visible BOOLEAN NOT NULL DEFAULT TRUE, music_affinity_consent BOOLEAN NOT NULL DEFAULT FALSE,
    adult_confirmed BOOLEAN NOT NULL DEFAULT FALSE, created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(), updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
CREATE TABLE IF NOT EXISTS concerts (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), artist VARCHAR(160) NOT NULL, venue VARCHAR(160) NOT NULL,
    city VARCHAR(80) NOT NULL, starts_at TIMESTAMP WITH TIME ZONE NOT NULL,
    status VARCHAR(20) NOT NULL DEFAULT 'scheduled' CHECK (status IN ('scheduled', 'cancelled', 'completed')),
    created_by_user_id UUID REFERENCES users(id) ON DELETE SET NULL, created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(), updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
CREATE TABLE IF NOT EXISTS concert_attendance_intents (
    ...
    concert_id UUID NOT NULL REFERENCES concerts(id) ON DELETE CASCADE, status VARCHAR(20) NOT NULL DEFAULT 'active' CHECK (status IN ('active', 'withdrawn')),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(), updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(), UNIQUE (user_id, concert_id)
);
CREATE TABLE IF NOT EXISTS companion_swipes (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), actor_user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    target_user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE, concert_id UUID NOT NULL REFERENCES concerts(id) ON DELETE CASCADE,
    action VARCHAR(20) NOT NULL CHECK (action IN ('pass', 'interested')), created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(), updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    CHECK (actor_user_id <> target_user_id), UNIQUE (actor_user_id, target_user_id, concert_id)
);
CREATE TABLE IF NOT EXISTS companion_matches (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), concert_id UUID NOT NULL REFERENCES concerts(id) ON DELETE CASCADE,
    user_low_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE, user_high_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(), CHECK (user_low_id < user_high_id), UNIQUE (concert_id, user_low_id, user_high_id)
);
CREATE TABLE IF NOT EXISTS user_blocks (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), blocker_user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    blocked_user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE, created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    CHECK (blocker_user_id <> blocked_user_id), UNIQUE (blocker_user_id, blocked_user_id)
);
CREATE TABLE IF NOT EXISTS user_reports (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), reporter_user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    reported_user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE, reason VARCHAR(40) NOT NULL, note VARCHAR(500) NOT NULL DEFAULT '',
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(), CHECK (reporter_user_id <> reported_user_id)
);
